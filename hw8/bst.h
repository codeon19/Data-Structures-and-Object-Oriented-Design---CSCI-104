...
class BinarySearchTree {
 ...
 public:
  ...
  void insert(const std::pair<const KeyType, ValueType> &new_item) {
    root = insertInTree(root, new_item);
    // counter++;
  }

  Node<KeyType, ValueType>* insertInTree(Node<KeyType, ValueType>* node, const std::pair<const KeyType, ValueType> &new_node) {
    
    // TODO
    // Feel free to modify any return values or parameters.
    // You can also choose to do your work in insert() instead.
    KeyType key = new_node.first;
    ValueType value = new_node.second;

    if(node == NULL)
    {
      Node<KeyType, ValueType>* newNode = new Node<KeyType, ValueType>(key, value, node);
      return newNode;
    }
    else if(node->getValue() < value)
    {
      Node<KeyType, ValueType>* rightNode = insertInTree(node->getRight(), new_node); 
    ...
    {
      Node<KeyType, ValueType>* leftNode = insertInTree(node->getLeft(), new_node);
  ...
  void print () const
  { 
    std::cout << "Root -> " << root->getKey() << endl;
    printRoot (root);
    std::cout << "\n";
  }
  ...
  protected:
    ...
    Node<KeyType, ValueType>* successor(Node<KeyType, ValueType>*& current) const
    {
      ...
      // fix error with root as the last item 
      if(!curr->getParent())
        return NULL;
